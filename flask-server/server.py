...
# Device configuration (loaded once)
DEVICE = "cpu"
ALLOW_CUDA = True
ALLOW_MPS = False
...
# Load the tokenizer and models once
tokenizer = CLIPTokenizer("./data/vocab.json", merges_file="./data/merges.txt")
model_file = "./data/v1-5-pruned-emaonly.ckpt"
models = model_loader.preload_models_from_standard_weights(model_file, DEVICE)
logging.info("Models and tokenizer loaded successfully.")

# Define a lock for GPU access
...
def generate():
    try:
        ...
        with gpu_lock:  # Acquire the GPU lock before using the GPU
            output_image = pipeline.generate(
                prompt=prompt,
                uncond_prompt=negative_prompt,
                input_image=input_image,
...
def list_images():
    try:
        ...
        if not os.path.exists(image_dir):
            logging.error(f"Directory does not exist: {image_dir}")
            return jsonify({"error": "Image directory does not exist"}), 404
        ...
        for filename in os.listdir(image_dir):
            ...
            if filename.lower().endswith(('.png', '.jpg', '.jpeg')):
                image_path = f"/origami_images/{filename}"
                images.append(image_path)
                logging.info(f"Added image path: {image_path}")
...
if __name__ == "__main__":
    app.run(debug=False, port=8080)  # Debug should be False in production
